...
class figure_detector(object):
	def __init__(self):
		...
		self.object_control = True
		self.black_cir_control = False
	...
	def cbFiguredetect(self, image):
		...
		if self.buf == False && self.state!= 0:
			self.transmit = 'S'
...
class ShapeDetector:
	...
	def detect(self, c):
		...
		c1 = cv2.convexHull(c)
		approx = cv2.approxPolyDP(c1, 0.1 * peri, True) #Approximates a polygonal curve with curve length (define how curve would be a line)
