...
for i=head+1:tail-1
    phi1(i)=i*i*coe(1)+i*coe(2)+coe(3);
end
phi = phi1;
...
%%%%% 需調整
range=15:33;
A=[thetadd(range) cos(phi(range)).*thetadd(range) -thetad(range) -45*ones(length(range), 1)];
...
C = [thetadd(range) thetad(range) -45*ones(length(range), 1)];
