bool CheckIfJoystick()
{
  if (str_receive[0] == '[' && str_receive[1] == 'j' && str_receive[2] == 'o' && str_receive[3] == 'y' && str_receive[4] == ':')
  {
    ...
    for (int i = 0 ; i < len_receive; i++) {
      if (str_receive[i] != '|' && change == false)
        speed_cmd[i] = str_receive[i];
      else if (str_receive[i] == '|')
        change = true;
      else if (str_receive[i] != '~' && change == true) {
        dir_cmd[j] = str_receive[i];
        j++;
      }
    }


    Speed_Need = atof(speed_cmd) / 50;
    Turn_Need = atof(dir_cmd);
    if (Speed_Need > 1.1 || Speed_Need < -1.1) {
      Speed_Need = 0;
    }
    if (Turn_Need > 90.1 || Turn_Need < -90.1) {
      Turn_Need = 0;
      Speed_Need = 0;
    }
    free(speed_cmd);
